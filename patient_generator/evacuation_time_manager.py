...
class EvacuationTimeManager:
    ...
    def _load_config(self, config_path: Union[str, Path]) -> Dict:
        ...
        try:
            ...
            msg = f"Evacuation configuration file not found: {config_path}"
    ...
    def _validate_config(self) -> None:
        ...
        for facility in expected_facilities:
            ...
            for triage in expected_triage:
                ...
                if min_hours < 0 or max_hours < 0:
                    msg = f"Negative time values not allowed: {facility} {triage}"
                    raise ValueError(msg)

                if min_hours > max_hours:
                    msg = f"min_hours > max_hours: {facility} {triage}"
                    raise ValueError(msg)
        ...
        for route in expected_routes:
            if route not in transit_times:
                msg = f"Missing transit times for route: {route}"
                raise ValueError(msg)

            for triage in expected_triage:
                if triage not in transit_times[route]:
                    msg = f"Missing triage {triage} for route {route}"
                    raise ValueError(msg)
                ...
                if min_hours < 0 or max_hours < 0:
                    msg = f"Negative transit time: {route} {triage}"
                    raise ValueError(msg)

                if min_hours > max_hours:
                    msg = f"min_hours > max_hours: {route} {triage}"
                    raise ValueError(msg)
        ...
        for triage in expected_triage:
            if triage not in kia_modifiers:
                msg = f"Missing KIA rate modifier for triage: {triage}"
                raise ValueError(msg)

            modifier = kia_modifiers[triage]
            if not isinstance(modifier, (int, float)) or modifier < 0:
                msg = f"Invalid KIA rate modifier for {triage}: {modifier}"
                raise ValueError(msg)
    ...
    def get_evacuation_time(self, facility: str, triage_category: str) -> float:
        ...
        if triage_category not in self.config["evacuation_times"][facility]:
            msg = f"Unknown triage category: {triage_category}"
            raise ValueError(msg)
    ...
    def get_transit_time(self, from_facility: str, to_facility: str, triage_category: str) -> float:
        ...
        if route_key not in self.config["transit_times"]:
            msg = f"Unknown transit route: {route_key}"
            raise ValueError(msg)
    ...
    def apply_triage_modifier(self, base_rate: float, triage_category: str, modifier_type: str = "kia") -> float:
        """
        ...
        if modifier_type == "kia":
            ...
            msg = f"Unknown modifier type: {modifier_type}"
            raise ValueError(msg)
