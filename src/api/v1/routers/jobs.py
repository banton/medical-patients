...
from src.api.v1.models import JobResponse, ErrorResponse
...
async def get_job_status(job_id: str, job_service: JobService = Depends(get_job_service)) -> JobResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve job {job_id}: {str(e)}"
        )
...
async def get_job_results(job_id: str, job_service: JobService = Depends(get_job_service)) -> JobResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve job results for {job_id}: {str(e)}"
        )
