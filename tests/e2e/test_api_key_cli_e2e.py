...
class TestCompleteAPIKeyLifecycle(TestAPIKeyCLIE2E):
    ...
    async def test_complete_api_key_lifecycle_workflow(self, cli_runner, patch_cli_database, repository, monkeypatch):
        ...
        usage_result = cli_runner.invoke(main, [
            'usage', key_id, '--format', 'json'
        ])
        ...
        limits_result = cli_runner.invoke(main, [
            'limits', key_id,
            '--patients', '2000',
            '--daily', '1000',
            '--hourly', '200'
        ...
        updated_show_result = cli_runner.invoke(main, [
            'show', key_id, '--format', 'json'
        ])
        ...
        extend_result = cli_runner.invoke(main, [
            'extend', key_id, '--days', '90'
        ])
        ...
        print("\n=== Step 7: Rotate Key ===")
        rotate_result = cli_runner.invoke(main, [
            'rotate', key_id, '--name', 'E2E Development Team v2'
        ])
        ...
        search_result = cli_runner.invoke(main, [
            'list', '--search', 'E2E Development', '--format', 'json'
        ])
        ...
        final_list_result = cli_runner.invoke(main, [
            'list', '--format', 'json'
        ])
    ...
    async def test_team_collaboration_workflow(self, cli_runner, patch_cli_database, repository, monkeypatch):
        ...
        for team in teams:
            create_args = [
                ...
                '--name', team['name'],
                '--email', team['email'],
                '--patients', str(team['patients']),
                '--daily', str(team['daily']),
                '--format', 'json'
            ]
        ...
        demo_result = cli_runner.invoke(main, [
            'list', '--demo', '--format', 'json'
        ])
...
class TestPerformanceWorkflows(TestAPIKeyCLIE2E):
    ...
    async def test_bulk_operations_performance(self, cli_runner, patch_cli_database, repository):
        ...
        search_result = cli_runner.invoke(main, [
            'list', '--search', 'Performance Test', '--format', 'json'
        ])
        ...
        csv_result = cli_runner.invoke(main, [
            'list', '--format', 'csv'
        ])
    ...
    async def test_memory_usage_large_dataset(self, cli_runner, patch_cli_database, repository):
        ...
        for i in range(num_keys):
            create_result = cli_runner.invoke(main, [
                'create',
                '--name', f'Memory Test Key {i:03d}',
                '--email', f'memory{i:03d}@test.com',
                '--format', 'json'
            ])
            assert create_result.exit_code == 0
...
class TestErrorRecoveryWorkflows(TestAPIKeyCLIE2E):
    ...
    async def test_error_recovery_workflow(self, cli_runner, patch_cli_database, repository, monkeypatch):
        ...
        invalid_result = cli_runner.invoke(main, [
            'show', 'invalid-key-id'
        ])
        ...
        noop_result = cli_runner.invoke(main, [
            'limits', key_id
        ])
        ...
        final_show_result = cli_runner.invoke(main, [
            'show', key_id, '--format', 'json'
        ])
    ...
    async def test_concurrent_modification_handling(self, cli_runner, patch_cli_database, repository):
        ...
        async def update_limits_async(max_patients, max_daily):
            ...
            return await repository.update_limits(
                key_id, 
                max_patients_per_request=max_patients,
...
class TestAutomationWorkflows(TestAPIKeyCLIE2E):
    ...
    async def test_automation_friendly_workflow(self, cli_runner, patch_cli_database, repository, monkeypatch):
        ...
        for env in test_environments:
            create_result = cli_runner.invoke(main, [
                'create',
                '--name', f'Automated {env.title()} Key',
                '--email', f'{env}@automation.test',
                '--patients', '500',
                '--daily', '200',
                '--format', 'json'
            ])
        ...
        for env, key_data in created_keys.items():
            ...
            await repository.update_usage(
                key_data['id'], 
                patients_generated=usage_amount[env],
                request_count=usage_amount[env] // 5
            )
        ...
        rotate_result = cli_runner.invoke(main, [
            'rotate', staging_key['id'], '--name', 'Automated Staging Key v2'
        ])
        ...
        cleanup_dry_result = cli_runner.invoke(main, [
            'cleanup', '--dry-run'
        ])
        ...
        json_report_result = cli_runner.invoke(main, [
            'list', '--format', 'json'
        ])
        ...
        csv_report_result = cli_runner.invoke(main, [
            'list', '--format', 'csv'
        ])
    ...
    async def test_ci_cd_integration_workflow(self, cli_runner, patch_cli_database, repository, monkeypatch):
        ...
        test_key_result = cli_runner.invoke(main, [
            'create',
            '--name', 'CI Test Key',
            '--demo',
            '--patients', '100',
            '--daily', '50',
            '--expires-days', '1',  # Short-lived for CI
            '--format', 'json'
        ])
        ...
        prod_key_result = cli_runner.invoke(main, [
            'create',
            '--name', 'Production Key',
        ...
        # Remove the test key
        cleanup_result = cli_runner.invoke(main, [
            'delete', test_key_id, '--confirm'
        ])
        ...
        verify_cleanup_result = cli_runner.invoke(main, [
            'show', test_key_id
        ])
        ...
        verify_prod_result = cli_runner.invoke(main, [
            'show', prod_key_data['id'], '--format', 'json'
        ])
