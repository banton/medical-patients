...
TestFramework.test('API banner should have correct text content', () => {
    ...
    assert(text.textContent.includes(expectedText), 
           `Banner text should include: "${expectedText}"`);
});

TestFramework.test('API banner link should point to documentation', () => {
    const link = querySelector('.api-banner__link');
    
    assert(link.getAttribute('href') === '/docs', 
           'Link should point to /docs');
    assert(link.textContent.includes('View API Documentation'), 
           'Link should contain documentation text');
});
...
TestFramework.test('API banner should have proper styling classes', () => {
    ...
    assert(banner.classList.contains('api-banner'), 
           'Banner should have api-banner class');
    assert(content.classList.contains('api-banner__content'), 
           'Content should have api-banner__content class');
    assert(text.classList.contains('api-banner__text'), 
           'Text should have api-banner__text class');
    assert(link.classList.contains('api-banner__link'), 
           'Link should have api-banner__link class');
});
...
TestFramework.test('API banner should be accessible', () => {
    ...
    assert(banner.getAttribute('aria-hidden') !== 'true', 
           'Banner should not be hidden from screen readers');
});
...
TestFramework.test('API banner link should open in same tab', () => {
    ...
    assert(link.getAttribute('target') !== '_blank', 
           'Internal documentation link should open in same tab');
});
...
TestFramework.test('API banner should persist on page interactions', () => {
    ...
    assert(initialDisplay === afterDisplay, 
           'Banner should remain visible after user interactions');
});
