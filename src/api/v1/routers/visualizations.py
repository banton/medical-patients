...
router = APIRouter(prefix="/api/visualizations", tags=["visualizations"], dependencies=[Depends(verify_api_key)])


@router.get("/dashboard-data")
async def get_dashboard_data(
    job_id: Optional[str] = Query(None), job_service: JobService = Depends(get_job_service)
) -> Dict[str, Any]:
...
async def get_patient_detail(
    patient_id: str, job_id: Optional[str] = Query(None), job_service: JobService = Depends(get_job_service)
) -> Dict[str, Any]:
