...
testFramework.test('Accordion should have proper section titles', () => {
    ...
    expectedTitles.forEach((title, index) => {
        ...
        assert(titleElement.textContent.includes(title), `Item ${index} should contain title "${title}"`);
    });
});
...
testFramework.test('Accordion should have validation status indicators', () => {
    ...
    items.forEach((item, index) => {
        ...
        const hasValidClass =
            status.classList.contains('accordion__status--valid') ||
...
testFramework.test('Accordion should collapse other sections when new one is opened', async () => {
    ...
    assert(!firstHeader.classList.contains('accordion__header--active'), 'First header should not be active');
