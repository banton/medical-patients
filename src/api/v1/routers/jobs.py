...
async def get_job_status(job_id: str, job_service: JobService = Depends(get_job_service)) -> Dict[str, Any]:
...
async def get_job_results(job_id: str, job_service: JobService = Depends(get_job_service)) -> Dict[str, Any]:
    """Get job results summary."""
    try:
        job = await job_service.get_job(job_id)

        if job.status.value != "completed":
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Job {job_id} is not completed yet")
