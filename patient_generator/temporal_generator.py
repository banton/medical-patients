...
class TemporalPatternGenerator:
    ...
    def __init__(self, warfare_patterns_path: str):
        ...
        with open(warfare_patterns_path, 'r') as f:
    ...
    def generate_timeline(self, 
        ...
        if not active_types:
            raise ValueError("No active warfare types selected")
    ...
    def _generate_precision_strike_pattern(self, total_patients: int,
        ...
        if preference == 'daylight':
            ...
            preferred_hours = list(range(0, 6)) + list(range(18, 24))
        else:
            ...
            for phase in phases:
                ...
                if phase_start <= phase_end:
                    if phase_start <= hour < phase_end:
                        ...
                        break
                else:  # Phase wraps around midnight
                    if hour >= phase_start or hour < phase_end:
                        in_phase = True
                        intensity = phase['intensity']
                        break
    ...
    def _get_special_events_for_day(self, day: int, special_events: Dict[str, bool],
        ...
        if special_events.get('mass_casualty'):
            # Generate mass casualty events on ~20% of days randomly
