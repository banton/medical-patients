...
class APIKeyRepository:
    ...
    def list_keys(
        self, active_only: bool = True, demo_only: Optional[bool] = None, limit: int = 100, offset: int = 0
    ) -> List[APIKey]:
        """
    ...
    def search_keys(self, search_term: str, limit: int = 50) -> List[APIKey]:
        ...
        return (
            self.db.query(APIKey)
            .filter(or_(APIKey.name.ilike(search_pattern), APIKey.email.ilike(search_pattern)))
    ...
    def get_usage_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_keys = self.db.query(APIKey).filter(APIKey.last_used_at >= since_date).all()
        ...
        active_keys = self.db.query(APIKey).filter(APIKey.is_active.is_(True)).count()
        demo_keys = self.db.query(APIKey).filter(APIKey.is_demo.is_(True)).count()
    ...
    def cleanup_expired_keys(self, dry_run: bool = True) -> List[str]:
        ...
        expired_keys = (
            self.db.query(APIKey)
            .filter(and_(APIKey.expires_at.isnot(None), APIKey.expires_at < datetime.utcnow()))
            .all()
        )
