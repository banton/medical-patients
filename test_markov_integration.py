...
def test_markov_chain_integration():
    ...
    result = subprocess.run([
        ...
        "--output", "output",
        "--formats", "json"
    ], capture_output=True, text=True, env={**os.environ, "ENABLE_MARKOV_CHAIN": "true"})
...
def analyze_triage_group(patients, triage_cat, expect_bypass_role1=False, expect_early_rtd=False):
    ...
    if expect_bypass_role1:
        if bypass_rate > 50:
            print(f"  ✅ T1 bypass pattern confirmed (>50% bypass Role1)")
        else:
    ...
    if expect_early_rtd:
        if rtd_role1_rate > 60:
            print(f"  ✅ T4 early RTD pattern confirmed (>60% RTD from Role1)")
        else:
