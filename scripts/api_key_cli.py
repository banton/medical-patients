    ...
    try:
        async with cli_app.session_factory() as session:
            ...
            if format == 'json':
                ...
                # Write header
                writer.writerow([
                    'ID', 'Key Prefix', 'Name', 'Email', 'Active', 'Demo', 
                    'Created', 'Expires', 'Last Used', 'Total Requests', 'Daily Requests'
                ])
    ...
    try:
        async with cli_app.session_factory() as session:
            ...
            if format == 'json':
                ...
                console.print(f"[bold]API Key Details[/bold]")
...
async def rotate(key_id, name):
    ...
    try:
        async with cli_app.session_factory() as session:
            ...
            console.print(f"[green]✓[/green] API key rotated successfully!")
            ...
            console.print(f"[yellow]⚠️  Store this new API key securely - it cannot be displayed again![/yellow]")
