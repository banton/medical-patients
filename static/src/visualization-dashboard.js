...
const generateMockData = () => {
    ...
    return {
        ...
        facility_usage: [
            { name: 'R1', capacity: 100, used: 70 },
            { name: 'R2', capacity: 80, used: 50 },
            { name: 'R3', capacity: 60, used: 30 },
            { name: 'R4', capacity: 40, used: 15 }
        ],
        timeline_analysis: [
            { day: 'Day 1', casualties: 30 },
            { day: 'Day 2', casualties: 45 },
            { day: 'Day 3', casualties: 20 },
            { day: 'Day 4', casualties: 15 }
        ],
        front_comparison: [
        ...
        injuryDistribution: [
            { name: 'Battle Trauma', value: 60 },
            { name: 'Disease', value: 25 },
            { name: 'Non-Battle', value: 15 }
        ],
        triageDistribution: [
            { name: 'T1', value: 10 },
            { name: 'T2', value: 40 },
...
const ExerciseDashboard = () => {
    ...
    const fetchDashboardData = async (jobIdToFetch) => {
        try {
            setLoading(true);
            setError(null);
            const url = jobIdToFetch
                ? `/api/visualizations/dashboard-data?job_id=${jobIdToFetch}`
                : '/api/visualizations/dashboard-data';
            const response = await fetch(url, {
                headers: window.API_CONFIG
                    ? window.API_CONFIG.getHeaders()
                    : {
    ...
    if (!LineChart && selectedJobId) {
        // Check one component as a proxy for all
        return (
            <div className="container mt-5">
                <div className="alert alert-danger" role="alert">
                    <AlertCircle /> Recharts library not loaded correctly. Dashboard cannot be displayed.
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="container mt-5">
                <div className="alert alert-info" role="alert">
                    No data available to display. Please select a job or generate new data.
                </div>
            </div>
        );
    ...
    const SummaryView = ({ data }) => (
        <div className="card">
            ...
            <div className="card-body">
                ...
                <p>
                    KIA Rate:{' '}
                    {data.summary?.kia_rate !== undefined ? (data.summary.kia_rate * 100).toFixed(1) + '%' : 'N/A'}
                </p>
                <p>
                    RTD Rate:{' '}
                    {data.summary?.rtd_rate !== undefined ? (data.summary.rtd_rate * 100).toFixed(1) + '%' : 'N/A'}
                </p>
    ...
    const PatientFlowSankey = ({ data }) => {
        if (!data || !data.nodes || !data.links || data.nodes.length === 0) {
            return (
                <div className="alert alert-warning">
                    Patient flow data is not available or incomplete for Sankey diagram.
                </div>
            );
        }
        ...
        const links = data.links
            .map((link) => ({
                source:
                    typeof link.source === 'number' ? link.source : nodes.findIndex((n) => n.name === link.source.name),
                target:
                    typeof link.target === 'number' ? link.target : nodes.findIndex((n) => n.name === link.target.name),
                value: link.value
            }))
            .filter(
                (link) =>
    ...
    const JobSelector = () => (
        <div className="mb-3">
            <label htmlFor="jobSelector" className="form-label">
                Select Job:
            </label>
            <select
                id="jobSelector"
    ...
    const ExportButtons = ({ data, tabName }) => {
        ...
        return (
            <div className="my-3 d-flex gap-2">
                <button className="btn btn-outline-primary btn-sm" onClick={() => printDashboard()}>
                ...
                <div className="card-body">
                    <div className="row g-3">
                        <div className="col-md-3">
                            <label htmlFor="nationalityFilter" className="form-label">
                                Nationality:
                            </label>
                            <select
                        ...
                        <div className="col-md-3">
                            <label htmlFor="injuryFilter" className="form-label">
                                Injury Type:
                            </label>
                            <select
                                id="injuryFilter"
                        ...
                        <div className="col-md-3">
                            <label htmlFor="triageFilter" className="form-label">
                                Triage Category:
                            </label>
                            <select
                                id="triageFilter"
                    ...
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">
                                Patient Detail: {patient.demographics?.given_name} {patient.demographics?.family_name}{' '}
                                (ID: {patient.id})
                            </h5>
                            <button type="button" className="btn-close" onClick={onClose}></button>
                        </div>
                        <div className="modal-body">
                            ...
                            <div className="mb-4">
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-md-6">
                                        <p>
                                            <strong>Blood Type:</strong> {patient.demographics?.blood_type}
                                        </p>
                                        <p>
                                            <strong>Weight:</strong> {patient.demographics?.weight} kg
                                        </p>
                                        <p>
                                            <strong>DOB:</strong> {patient.demographics?.birthdate}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mb-4">
                                ...
                                <p>
                                    <strong>Primary Condition:</strong> {patient.primary_condition?.display} (
                                    {patient.primary_condition?.severity})
                                </p>
                                <p>
                                    <strong>Triage Category:</strong> {patient.triage_category}
                                </p>
                                <p>
                                    <strong>Injury Type:</strong> {patient.injury_type}
                                </p>
                                        ...
                                        {patient.treatment_history.map((treatment, idx) => (
                                            <div key={idx} className="timeline-item mb-3">
                                                <div
                                                    ...
                                                    style={{ backgroundColor: idx === 0 ? '#0d6efd' : '#6c757d' }}
                                                ></div>
        ...
        <div className="container-fluid mt-3" id="printable-dashboard">
            <div className="row">
                <div className="col-12">
                    <h2 className="mb-3">
                        <FilePieChart /> Enhanced Visualization Dashboard
                    </h2>
                    <JobSelector />
                    {loading && selectedJobId && (
                        <div className="text-center my-3">
                            ...
                            <p>Loading data for {selectedJobId}...</p>
                        </div>
                    )}
                    {error && (
            ...
            {displayedData && (
                <>
                    ...
                    <ul className="nav nav-tabs mb-3">
                        ...
                        <li className="nav-item">
                            <a
                                className={`nav-link ${activeTab === 'patientFlow' ? 'active' : ''}`}
                                href="#"
                                onClick={(e) => {
                                    e.preventDefault();
                                    setActiveTab('patientFlow');
                                }}
                            >
                                Patient Flow
                            </a>
                        ...
                        <li className="nav-item">
                            <a
                                className={`nav-link ${activeTab === 'facilityUsage' ? 'active' : ''}`}
                                href="#"
                                onClick={(e) => {
                                    e.preventDefault();
                                    setActiveTab('facilityUsage');
                                }}
                            >
                                Facility Usage
                            </a>
                        ...
                        <li className="nav-item">
                            <a
                                className={`nav-link ${activeTab === 'nationality' ? 'active' : ''}`}
                                href="#"
                                onClick={(e) => {
                                    e.preventDefault();
