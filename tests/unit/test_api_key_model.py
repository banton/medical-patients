...
class TestAPIKeyUsabilityLogic:
    ...
    def test_is_usable_active_and_not_expired(self):
        ...
        api_key = APIKey(key="usable", name="Usable", is_active=True, expires_at=datetime.utcnow() + timedelta(days=30))
    ...
    def test_is_usable_inactive_and_expired(self):
        ...
        api_key = APIKey(key="both", name="Both", is_active=False, expires_at=datetime.utcnow() - timedelta(days=1))
        assert api_key.is_usable() is False
...
class TestAPIKeyLimitChecking:
    ...
    def test_check_daily_limit_unlimited(self):
        ...
        api_key = APIKey(key="unlimited", name="Unlimited", max_requests_per_day=None, daily_requests=10000)
    ...
    def test_check_daily_limit_under_limit(self):
        ...
        api_key = APIKey(key="limited", name="Limited", max_requests_per_day=100, daily_requests=50)
        assert api_key.check_daily_limit() is True

    def test_check_daily_limit_at_limit(self):
        """Test daily limit check at exactly the limit."""
        api_key = APIKey(key="at_limit", name="At Limit", max_requests_per_day=100, daily_requests=100)
        assert api_key.check_daily_limit() is False

    def test_check_daily_limit_over_limit(self):
        """Test daily limit check over the limit."""
        api_key = APIKey(key="over_limit", name="Over Limit", max_requests_per_day=100, daily_requests=150)
        assert api_key.check_daily_limit() is False
...
class TestAPIKeyDailyResetLogic:
    ...
    def test_reset_daily_counters(self, mock_datetime):
        ...
        api_key = APIKey(
            key="reset_test", name="Reset Test", daily_requests=50, last_reset_at=datetime(2024, 1, 14, 8, 0, 0)
        )
...
class TestAPIKeyUsageTracking:
    ...
    def test_record_usage(self, mock_datetime):
        ...
        api_key = APIKey(
            key="usage_test", name="Usage Test", total_requests=100, total_patients_generated=2000, daily_requests=10
        )
    ...
    def test_record_usage_zero_patients(self):
        ...
        api_key = APIKey(
            key="zero_test", name="Zero Test", total_requests=0, total_patients_generated=0, daily_requests=0
        )
...
class TestAPIKeyDataMethods:
    ...
    def test_to_dict_with_none_values(self):
        ...
        api_key = APIKey(
            key="none_test", name="None Test", email=None, expires_at=None, last_used_at=None, max_requests_per_day=None
        )
