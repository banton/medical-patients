...
class TestCachedDemographicsService:
    ...
    async def test_get_demographics_data_from_cache(self, mock_cache_service, mock_settings):
        ...
        with patch(
            "src.domain.services.cached_demographics_service.get_cache_service", return_value=mock_cache_service
        ):
            # Execute
            ...
            mock_cache_service.get.assert_called_once_with(CachedDemographicsService.DEMOGRAPHICS_JSON_CACHE_KEY)
    ...
    async def test_get_demographics_data_cache_miss(self, mock_cache_service, mock_settings):
        ...
        with patch.object(service, "_load_demographics_data", return_value=expected_data), patch(
            "src.domain.services.cached_demographics_service.get_cache_service", return_value=mock_cache_service
        ):
            # Execute
            result = await service.get_demographics_data()
            ...
            mock_cache_service.set.assert_called_once_with(
                CachedDemographicsService.DEMOGRAPHICS_JSON_CACHE_KEY, expected_data, ttl=86400
            )
    ...
    async def test_get_demographics_data_cache_disabled(self):
        ...
        with patch("src.domain.services.cached_demographics_service.settings.CACHE_ENABLED", False), patch.object(
            service, "_load_demographics_data", return_value=expected_data
        ):
            # Execute
            result = await service.get_demographics_data()
    ...
    async def test_invalidate_cache(self, mock_cache_service, mock_settings):
        ...
        with patch(
            ...
            mock_cache_service.delete.assert_called_once_with(CachedDemographicsService.DEMOGRAPHICS_JSON_CACHE_KEY)
...
class TestCachedMedicalService:
    ...
    async def test_get_medical_data_from_cache(self, mock_cache_service):
        ...
        with patch("src.domain.services.cached_medical_service.settings.CACHE_ENABLED", True), patch(
            "src.domain.services.cached_medical_service.get_cache_service", return_value=mock_cache_service
        ):
            # Execute
            ...
            # Verify
            assert result == cached_data
            mock_cache_service.get.assert_called_once_with(CachedMedicalService.MEDICAL_DATA_CACHE_KEY)
    ...
    async def test_get_medical_data_cache_miss(self, mock_cache_service):
        ...
        expected_data = {"battle_trauma_conditions": [{"code": "125670008", "display": "War injury"}]}
        ...
        with patch("src.domain.services.cached_medical_service.settings.CACHE_ENABLED", True), patch(
            ...
            assert result == expected_data
            mock_cache_service.set.assert_called_once_with(
                CachedMedicalService.MEDICAL_DATA_CACHE_KEY, expected_data, ttl=86400
            )
    ...
    async def test_get_condition_no_caching(self):
        ...
        with patch("src.domain.services.cached_medical_service.settings.CACHE_ENABLED", True), patch.object(
            service, "_get_condition_generator", return_value=mock_generator
        ):
            # Execute
    ...
    async def test_get_multiple_conditions_no_caching(self):
        ...
        with patch.object(service, "_get_condition_generator", return_value=mock_generator):
            ...
            mock_generator.generate_multiple_conditions.assert_called_once_with("BATTLE_TRAUMA", "T1", 2)
