            ...
            "teams": [
                {
                    ...
                    "limits": {"patients": 500, "daily": 200}
                },
                {
                    "name": "Backend Team",
                    "email": "backend@company.com",
                    "limits": {"patients": 2000, "daily": 1000}
                },
                {
                    "name": "QA Team",
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "cli_unit: Unit tests for CLI components"
    )
    config.addinivalue_line(
        "markers", "cli_integration: Integration tests with database"
    )
    config.addinivalue_line(
        "markers", "cli_e2e: End-to-end workflow tests"
    )
    config.addinivalue_line(
        "markers", "cli_performance: Performance and load tests"
    )
    config.addinivalue_line(
        "markers", "cli_automation: Automation and scripting tests"
    )