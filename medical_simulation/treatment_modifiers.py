...
class TreatmentModifiers:
    ...
    def _define_facility_capabilities(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "role1": ["tourniquet", "pressure_dressing", "hemostatic_agent",
                     "iv_fluids", "morphine", "antibiotics"],
            "role2": ["tourniquet", "pressure_dressing", "hemostatic_agent",
                     "iv_fluids", "morphine", "antibiotics",
                     "blood_transfusion", "chest_tube", "surgical_stabilization"],
            "role3": ["tourniquet", "pressure_dressing", "hemostatic_agent",
                     "iv_fluids", "morphine", "antibiotics",
                     "blood_transfusion", "chest_tube", "surgical_stabilization",
                     "major_surgery", "organ_repair"],
    ...
    def get_available_treatments(
        ...
        """
        base_treatments = self.facility_capabilities.get(
            facility_type.lower(), []
        )

        # Filter by patient condition if specified
        if patient_condition:
            if "hemorrhage" in patient_condition.lower():
                return [t for t in base_treatments
                       if t in ["tourniquet", "pressure_dressing",
                               "hemostatic_agent", "blood_transfusion"]]
            elif "respiratory" in patient_condition.lower():
                return [t for t in base_treatments
                       if t in ["chest_tube", "surgical_stabilization"]]
    ...
    def get_treatment_priority(
        ...
        if patient_health < 50:
            ...
            priorities.extend([t for t in stabilize
                             if t in available_treatments and t not in priorities])

        # Comfort and infection prevention
        comfort = ["morphine", "antibiotics", "pressure_dressing"]
        priorities.extend([t for t in comfort
                         if t in available_treatments and t not in priorities])
...
if __name__ == "__main__":
    ...
    health, deterioration, info = tm.apply_treatment(
        "tourniquet", current_health=30, current_deterioration=25
    )
