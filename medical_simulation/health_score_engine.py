    ...
    def calculate_health_timeline(
        ...
        for hour in range(duration_hours + 1):
            ...
            for mod in active_modifiers:
                if mod.get("type") == "treatment":
                    effective_rate *= mod.get("modifier", 1.0)
                elif mod.get("type") == "environment":
                    effective_rate *= mod.get("modifier", 1.0)
            ...
            if cliff_config.get("enabled", False) and hour > 0:
                if random.random() < cliff_config.get("probability_per_hour", 0.05):
