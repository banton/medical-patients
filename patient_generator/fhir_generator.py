...
class FHIRBundleGenerator:
    ...
    def create_patient_bundle(self, patient):
        ...
        bundle = {
            ...
            "type": "collection",
            "timestamp": timestamp,
            "extension": [{"url": "http://example.org/fhir/StructureDefinition/nfc-tag-id", "valueString": nfc_id}],
    ...
    def _get_cached_coding(self, system, code, display):
        ...
        if key not in self._coding_cache:
            self._coding_cache[key] = [{"system": system, "code": code, "display": display}]
    ...
    def _create_patient_resource(self, patient):
        ...
        if demographics and "family_name" in demographics and "given_name" in demographics:
            patient_resource["name"] = [{"family": demographics["family_name"], "given": [demographics["given_name"]]}]
        ...
        if patient.nationality:
            patient_resource["extension"].append(
                {"url": "http://example.org/fhir/StructureDefinition/nationality", "valueString": patient.nationality}
            )
        ...
        if demographics and "id_number" in demographics and demographics["id_number"]:
            system = (
                ...
                if patient.nationality == "NLD"
                else "http://example.org/identifier"
            )
            patient_resource["identifier"].append({"system": system, "value": demographics["id_number"]})
        ...
        for visit in patient.treatment_history:
            ...
            for treatment in visit.get("treatments", []):
                procedure = self._create_procedure_resource(
                    patient_id, treatment["code"], treatment["display"], visit["date"]
                )
            ...
            for obs in visit.get("observations", []):
                observation = self._create_observation_resource(
                    patient_id, obs["code"], obs["display"], obs["value"], obs.get("unit"), visit["date"]
                )
    ...
    def _create_condition_resource(self, condition, patient_id):
        ...
        condition_resource = {
            ...
            "code": {"coding": self._get_cached_coding("http://snomed.info/sct", code, display)},
            "clinicalStatus": {
                "coding": self._get_cached_coding(
        ...
        if severity and severity_code:
            condition_resource["severity"] = {
                "coding": self._get_cached_coding("http://snomed.info/sct", severity_code, severity)
            }
    ...
    def _create_procedure_resource(self, patient_id, code, display, date):
        ...
        if date:
            date_str = date.isoformat() if isinstance(date, (datetime.datetime, datetime.date)) else str(date)
    ...
    def _create_observation_resource(self, patient_id, code, display, value, unit, date):
        ...
        observation = {
            ...
            "status": "final",
            "subject": {"reference": f"Patient/{patient_id}"},
            "code": {"coding": self._get_cached_coding("http://loinc.org", code, display)},
        }
    ...
    def _create_blood_type_observation(self, patient_id, blood_type):
        ...
        blood_type_codes = {"A": "112144000", "B": "165743006", "AB": "112149005", "O": "58460004"}
        ...
        return {
            ...
            "status": "final",
            "subject": {"reference": f"Patient/{patient_id}"},
            "code": {"coding": self._get_cached_coding("http://loinc.org", "883-9", "ABO group")},
            "valueCodeableConcept": {
                "coding": self._get_cached_coding("http://snomed.info/sct", code, f"Blood group {blood_type}")
            },
    ...
    def _create_weight_observation(self, patient_id, weight):
        ...
        return {
            ...
            "status": "final",
            "subject": {"reference": f"Patient/{patient_id}"},
            "code": {"coding": self._get_cached_coding("http://loinc.org", "29463-7", "Body weight")},
